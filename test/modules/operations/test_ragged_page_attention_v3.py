...
@pytest.mark.skipif(
    device_platform() != "gpu" or not _HAS_CUTLASS,
    reason="GPU + CUTLASS required for CuTe cross-backend comparison",
)
def test_ragged_page_attention_v3_cute_matches_xla_on_gpu():
    batch = make_dummy_rpa_inputs(
        rng_seed=2,
        num_seqs=3,
        pages_per_seq=4,
    ...
    kv_cache_xla = batch["kv_cache"].copy()
    kv_cache_cute = batch["kv_cache"].copy()
    ...
    out_cute, cache_cute = ragged_page_attention_v3(
        batch["queries"],
        batch["keys"],
        batch["values"],
        kv_cache_cute,
    ...
    assert_allclose(out_cute, out_xla, atol=0.06)
    assert_allclose(cache_cute, cache_xla, atol=0.0)
