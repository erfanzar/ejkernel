...
def test_flash_mla_basic_runs_on_gpu():
    ...
    w_kc = jax.random.normal(jax.random.PRNGKey(2), (kv_lora_rank, kv_heads, head_dim), dtype=jnp.float32).astype(
        jnp.bfloat16
    )
    w_vc = jax.random.normal(jax.random.PRNGKey(3), (kv_lora_rank, kv_heads, head_dim), dtype=jnp.float32).astype(
        jnp.bfloat16
    )
