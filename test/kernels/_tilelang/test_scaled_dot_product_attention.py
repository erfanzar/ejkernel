...
"""TileLang parity tests for scaled_dot_product_attention."""
...
def test_scaled_dot_product_attention_matches_flash_attention():
    B, N, H, D = 1, 32, 2, 64
    key = jax.random.PRNGKey(_SEED)
    k1, k2, k3 = jax.random.split(key, 3)
    q, k, v = _randn(k1, (B, N, H, D)), _randn(k2, (B, N, H, D)), _randn(k3, (B, N, H, D))
    out_tl = _tl("scaled_dot_product_attention")(q, k, v, causal=False)
    out_fa = _tl("flash_attention")(q, k, v, causal=False)
    assert _max_abs(out_tl, out_fa) < 1e-6


def test_scaled_dot_product_attention_surfaces_features():
    B, N, H, D = 1, 64, 4, 64
    ...
    tl, xla = _tl("scaled_dot_product_attention"), _xla("scaled_dot_product_attention")
